...
def analyze_geometry(mol):
    ...
    for a1, a2 in bonded:
        for a3 in neighbors.get(a2, []):
            if a3 == a1: continue
            p1, p2, p3 = np.array(conf.GetAtomPosition(a1)), \
                         np.array(conf.GetAtomPosition(a2)), \
                         np.array(conf.GetAtomPosition(a3))
...
def main():
    ...
    # Trigger calculation automatically on new file
    ...
    if trigger:
        ...
        # Lookup DB
        db_energy, db_avail, db_source, _ = find_database_energy(mol)
        # Display results
